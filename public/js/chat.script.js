...
var chat = {
        ...
        this.data.wSock.onmessage = function (event) {
            ...
            switch (d.code) {
                ...
                case 3:
                    ...
                    break;
                case 4: //页面初始化
                    initConfiguare.initPage(d.data);
                    break;
                case 5:
                    if (d.data.mine) {
                        chat.displayError('chatErrorMessage_logout', d.msg, 1);
    ...
    changeRoom: function (obj) {
        ...
        var json = {
            ...
            "oldroomid": oldroomid,
            "roomid": this.data.crd,
            "email": chat.data.storage.getItem("email")
        };
        chat.wsSend(JSON.stringify(json));
    ...
    //展示错误
    //  'chatErrorMessage_logout',"请输入昵称和Email才可以参与群聊哦～",1
    ...
    sendSecretMsg: function () {
        ...
        var json = {
            "type": chat.data.type,
            "send_name": chat.data.name,
            "send_avatar": chat.data.avatar,
            "message": text,
            "c": 'text',
            "receive_fd": this.data.secretToUser
        };
        chat.wsSend(JSON.stringify(json));
        return true;
    },
