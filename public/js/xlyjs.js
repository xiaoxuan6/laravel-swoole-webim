        ...
        upload:
            function (action, callbackurl, callback, check) {
                var option = {
                    ...
                    method: {
                        callback: function (that) {
                            ...
                            // console.log('that', that.contentWindow);
                            // var text = that.contentWindow.document.body.innerHTML;
                            // console.log('text', that.contentWindow.document.body);
                            // console.log('text11', text);
                            // var text = that.contentWindow.find('body').innerHTML;
                ...
                if ((navigator.userAgent.indexOf('MSIE') >= 0) && (navigator.userAgent.indexOf('Opera') < 0)) {
                    $.app.method.uploadOption.dialog = $('#' + $.app.method.uploadOption.id).dialog({
                        title: '请选择文件',
